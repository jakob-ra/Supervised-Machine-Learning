...
pack_vhat_laplace <- svmmaj(X,y,lambda = opt_lambda,hinge="quadratic",
...
- Prediction success alternative kernels: measures of classificationpackage with alternative kernels
...
We find that we can succesfully predict the calls not leading to sign-up, while it is harder to predict the ones that do sign up for one. If one would use the whole data set, this could most likely be improved. Surprisingly variables like the month in which customers are contacted have a relatively large influence on the predicted value and calling customers on the phone seems to be counterproductive.
