...
![Results from 9-fold cross validation](/Users/Maja/Documents/GitHub/Supervised-Machine-Learning/HW3/manual_rmse.png)
...
In this paper we wanted to investigate whether we could predict an airline's revenue passenger miles (RPM), using fuel price and airline characteristics. Using kernel ridge regression, we found that XXXXXXXXXXXXXXXXX

Although we achieve predictions, we are in this analysis not distinguishing between what predictors are impoartant, their relative importance and the direction of that importance. Therefore, we cannot make any statements about the distinct explanatory factors. Nevertheless, predicitions can still be of use to planners in the aviation industry for their prospective business decisions. 

Further, the analysis is sensitive to the decisions we have made - we chose two particular kernels beacause of their inherent benefits, but it is not clear how to optimally select a kernel and by our choice we introduce a functional form preference. We tune one of our parameters but choose the other, meaning there is as a risk of supobtimal results. 
